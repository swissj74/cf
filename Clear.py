...
for rr_alpha in [0.1, 1, 10]:
    ...
    #print('Coefficients: \n', rr_model.coef_)
    # The mean square error
    rss = np.mean((rr_model.predict(cf_pd_test[my_features]) - cf_pd_test["BOX"]) ** 2)
    # Explained variance score: 1 is perfect prediction
    variance = rr_model.score(cf_pd_test[my_features], cf_pd_test["BOX"])
    # Predict with all features
    predicted_box_office = rr_model.predict(cf_in[my_features])[0]
    model = 'rr model'
    print(model, '\t', rss, '\t', variance , '\t', predicted_box_office)
...
#print('Coefficients: \n', rr_cv_model.coef_)
# The mean square error
rss = np.mean((rr_cv_model.predict(cf_pd_test[my_features]) - cf_pd_test["BOX"]) ** 2)
# Explained variance score: 1 is perfect prediction
variance = rr_cv_model.score(cf_pd_test[my_features], cf_pd_test["BOX"])
# Predict with all features
predicted_box_office = rr_cv_model.predict(cf_in[my_features])[0]
model = 'rr cv model'
...
# The mean square error
rss = np.mean((lasso_cv_model.predict(cf_pd_test[my_features]) - cf_pd_test["BOX"]) ** 2)
# Explained variance score: 1 is perfect prediction
variance = lasso_cv_model.score(cf_pd_test[my_features], cf_pd_test["BOX"])
# Predict with all features
predictedBoxOffice = lasso_cv_model.predict(cf_in[my_features])[0]
model = 'lasso cv model'

print(model, '\t', rss, '\t', variance , '\t', predictedBoxOffice)
...
# The mean square error
rss = np.mean((rr_cv_lasso_model.predict(cf_pd_test[my_features_lasso]) - cf_pd_test["BOX"]) ** 2)
# Explained variance score: 1 is perfect prediction
variance = rr_cv_lasso_model.score(cf_pd_test[my_features_lasso], cf_pd_test["BOX"])
# Predict with all features
predictedBoxOffice =  rr_cv_lasso_model.predict(cf_in[my_features_lasso])[0]
model = 'rr cv model'
print(model, '\t', rss, '\t', variance , '\t', predictedBoxOffice)
...
# The mean square error
rss = np.mean((elastic_net_model.predict(cf_pd_test[my_features]) - cf_pd_test["BOX"]) ** 2)
# Explained variance score: 1 is perfect prediction
variance = elastic_net_model.score(cf_pd_test[my_features], cf_pd_test["BOX"])
# Predict with all features
predictedBoxOffice = elastic_net_model.predict(cf_in[my_features])[0]
model = 'elastic net model'
print(model, '\t', rss, '\t', variance , '\t', predictedBoxOffice)
...
#print('Coefficients: \n', elastic_net_model.coef_)
# The mean square error
rss = np.mean((elastic_net_model.predict(cf_pd_test[my_features_lasso]) - cf_pd_test["BOX"]) ** 2)
# Explained variance score: 1 is perfect prediction
variance = elastic_net_model.score(cf_pd_test[my_features_lasso], cf_pd_test["BOX"])
# Predict with all features
predictedBoxOffice = elastic_net_model.predict(cf_in[my_features_lasso])[0]
model = 'elastic net model'
print(model, '\t', rss, '\t', variance , '\t', predictedBoxOffice)
...
# The mean square error
rss = np.mean((lr_model_small.predict(cf_pd_test[my_features_small]) - cf_pd_test["BOX"]) ** 2)
# Explained variance score: 1 is perfect prediction
variance = lr_model_small.score(cf_pd_test[my_features_small], cf_pd_test["BOX"])
# Predict with all features
predictedBoxOffice = lr_model_small.predict(cf_in[my_features_small])[0]
model = 'lr model small'
print(model, '\t', rss, '\t', variance , '\t', predictedBoxOffice)
...
# The mean square error
rss = np.mean((lasso_lars_model.predict(cf_pd_test[my_features]) - cf_pd_test["BOX"]) ** 2)
# Explained variance score: 1 is perfect prediction
variance = lasso_lars_model.score(cf_pd_test[my_features], cf_pd_test["BOX"])
# Predict with all features
predictedBoxOffice = lasso_lars_model.predict(cf_in[my_features])[0]
model = 'lasso lars model'
print(model, '\t', rss, '\t', variance , '\t', predictedBoxOffice)
...
# The mean square error
rss = np.mean((lr_model_small_lasso_lars.predict(cf_pd_test[my_lasso_lars_features]) - cf_pd_test["BOX"]) ** 2)
# Explained variance score: 1 is perfect prediction
variance = lr_model_small_lasso_lars.score(cf_pd_test[my_lasso_lars_features], cf_pd_test["BOX"])
# Predict with all features
predictedBoxOffice = lr_model_small_lasso_lars.predict(cf_in[my_lasso_lars_features])[0]
model = 'lr model small lasso lars'
print(model, '\t', rss, '\t', variance , '\t', predictedBoxOffice)
...
#print('Coefficients: \n', br_model.coef_)
# The mean square error
rss = np.mean((br_model.predict(cf_pd_test[my_lasso_lars_features]) - cf_pd_test["BOX"]) ** 2)
# Explained variance score: 1 is perfect prediction
variance = br_model.score(cf_pd_test[my_lasso_lars_features], cf_pd_test["BOX"])
# Predict with all features
predictedBoxOffice = br_model.predict(cf_in[my_lasso_lars_features])[0]
model = 'br model'
print(model, '\t', rss, '\t', variance , '\t', predictedBoxOffice)
...
#print('Coefficients: \n', br_model.coef_)
# The mean square error
rss = np.mean((br_model.predict(cf_pd_test[my_features_lasso]) - cf_pd_test["BOX"]) ** 2)
# Explained variance score: 1 is perfect prediction
variance = br_model.score(cf_pd_test[my_features_lasso], cf_pd_test["BOX"])
# Predict with all features
predictedBoxOffice = br_model.predict(cf_in[my_features_lasso])[0]
model = 'br model'
print(model, '\t', rss, '\t', variance , '\t', predictedBoxOffice)
...
#print('Coefficients: \n', rr_cv_model.coef_)
# The mean square error
rss = np.mean((rr_cv_model.predict(cf_pd_test[my_features_lasso]) - cf_pd_test["BOX"]) ** 2)
# Explained variance score: 1 is perfect prediction
variance = rr_cv_model.score(cf_pd_test[my_features_lasso], cf_pd_test["BOX"])
# Predict with all features
predictedBoxOffice = rr_cv_model.predict(cf_in[my_features_lasso])[0]
print(model, '\t', rss, '\t', variance , '\t', predictedBoxOffice)
...
#print('Coefficients: \n', rr_cv_model.coef_)
# The mean square error
rss = np.mean((rr_cv_model.predict(cf_pd_test[my_lasso_lars_features]) - cf_pd_test["BOX"]) ** 2)
# Explained variance score: 1 is perfect prediction
variance = rr_cv_model.score(cf_pd_test[my_lasso_lars_features], cf_pd_test["BOX"])
# Predict with all features
predictedBoxOffice = rr_cv_model.predict(cf_in[my_lasso_lars_features])[0]
model = 'rr cv model'
print(model, '\t', rss, '\t', variance , '\t', predictedBoxOffice)
